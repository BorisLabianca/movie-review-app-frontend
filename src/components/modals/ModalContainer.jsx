const ModalContainer = ({ visible, children, onClose, ignoreContainer }) => {
  const handleClick = (event) => {
    if (event.target.id === "modal-container") onClose && onClose();
  };
  ...
  return (
    <div
      onClick={handleClick}
      id="modal-container"
      className="fixed inset-0 bg-primary bg-opacity-50 dark:bg-white dark:bg-opacity-50 backdrop-blur-sm flex items-center justify-center"
    >
      {renderChildren()}
    </div>
  );
